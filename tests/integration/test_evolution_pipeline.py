...
class TestEvolutionPipeline:
    ...
    async def test_full_evolution_loop(
        ...
        assert (
            len(migrant_programs) == 0
        ), f"Found programs with _migrant_ suffix: {migrant_programs}"

        # Print stats for debugging
        total_programs = len(controller.database.programs)
        evolved_programs = [
            p for p in controller.database.programs.values() if p.iteration_found > 0
        ]
        print(
            f"Evolution results: {total_programs} total programs, {len(evolved_programs)} evolved programs"
        )
    ...
    async def test_evolution_with_small_model_succeeds(
        ...
        assert (
            best_program is not None or len(controller.database.programs) >= 1
    ...
    async def test_best_program_tracking(
        ...
        if controller.database.best_program_id:
            ...
            if best_program:
                assert (
                    best_program.id == controller.database.best_program_id
