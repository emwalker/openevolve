...
class TestEvolutionTracer(unittest.TestCase):
    ...
    def test_tracer_disabled(self):
        ...
        tracer.log_trace(
            iteration=1, parent_program=self.parent_program, child_program=self.child_program
        )
    ...
    def test_json_format(self):
        ...
        tracer = EvolutionTracer(
            output_path=str(output_path), format="json", include_prompts=False, buffer_size=1
        )
    ...
    def test_statistics(self):
        ...
        tracer = EvolutionTracer(output_path=Path(self.temp_dir) / "trace.jsonl", buffer_size=10)
        ...
        # Log decline
        tracer.log_trace(
            iteration=2, parent_program=self.child_program, child_program=self.parent_program
        )
    ...
    def test_buffer_flushing(self):
        ...
        tracer = EvolutionTracer(output_path=str(output_path), format="jsonl", buffer_size=2)
        ...
        tracer.log_trace(
            iteration=2, parent_program=self.parent_program, child_program=self.child_program
        )
    ...
    def test_context_manager(self):
        ...
        with EvolutionTracer(output_path=str(output_path), buffer_size=10) as tracer:
            tracer.log_trace(
                iteration=1, parent_program=self.parent_program, child_program=self.child_program
            )
    ...
    def test_artifact_logging(self):
        ...
        tracer = EvolutionTracer(output_path=str(output_path), include_code=False, buffer_size=1)

        artifacts = {"execution_output": "Result: 42", "error": None, "timing": 0.123}
...
class TestCheckpointExtraction(unittest.TestCase):
    ...
    def _create_test_program(
        self, prog_id, parent_id=None, iteration=0, metrics=None, generation=0
    ):
        ...
        program_data = {
            ...
            "metadata": {"island": iteration % 3, "changes": f"Changes for {prog_id}"},
        }
    ...
    def test_extract_with_code_inclusion(self):
        ...
        traces = extract_evolution_trace_from_checkpoint(self.checkpoint_dir, include_code=True)
    ...
    def test_extract_without_code(self):
        ...
        traces = extract_evolution_trace_from_checkpoint(self.checkpoint_dir, include_code=False)
    ...
    def test_save_extracted_traces_jsonl(self):
        ...
        traces = extract_evolution_trace_from_checkpoint(
            self.checkpoint_dir, output_path=str(output_path), format="jsonl"
        )
    ...
    def test_save_extracted_traces_json(self):
        ...
        traces = extract_evolution_trace_from_checkpoint(
            self.checkpoint_dir, output_path=str(output_path), format="json"
        )
