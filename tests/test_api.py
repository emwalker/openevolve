...
""")
        ...
        proc = subprocess.run(
            [sys.executable, test_script], capture_output=True, text=True, timeout=10
        )
    ...
    def test_evolve_function_evaluator_works_in_subprocess(self):
        ...
        with unittest.mock.patch("openevolve.api.run_evolution") as mock_run:
            mock_run.return_value = EvolutionResult(
                best_program=None, best_score=1.0, best_code="", metrics={}, output_dir=None
            )
        ...
        self.assertIsInstance(
            evaluator_code, str, "evolve_function should pass evaluator as code string"
        )
